...
namespace nih {
/*
 * \brief A json reader, currently error checking and utf-8 is not fully
 * supported.
 */
class JsonReader {
protected:
...
protected:
  ...
  /* \brief Consume next character without first skipping empty space, throw
   * when the next character is not the expected one.
   */
