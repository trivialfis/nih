...
class StringRefImpl {
 ...
 public:
  StringRefImpl(std::string &str) : _chars{&str[0]}, _size{str.size()} {}  // NOLINT
  ...
  template <typename CharU, std::enable_if_t<!std::is_same_v<CharT, CharU>> * = nullptr>
  StringRefImpl(StringRefImpl<CharU> that) : _chars{that.data()}, _size{that.size()} {}
