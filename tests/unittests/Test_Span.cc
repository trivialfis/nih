...
struct TestTestStatus {
  ...
  void operator()(int _idx) { SPAN_ASSERT_TRUE(false, status_); }
};
...
struct TestAsBytes {
  ...
  void operator()(int _idx) {
...

    {
      ...
      SPAN_ASSERT_TRUE(
          static_cast<const void*>(bs.data()) == static_cast<const void*>(s.data()),
          status_);
    }
...
struct TestAsWritableBytes {
  ...
  void operator()(int _idx) {
...

    {
      ...
      SPAN_ASSERT_TRUE(bs.size_bytes() == 0, status_);
      SPAN_ASSERT_TRUE(bs.data() == nullptr, status_);
      SPAN_ASSERT_TRUE(static_cast<void*>(bs.data()) == static_cast<void*>(s.data()),
                       status_);
    }
...
TEST(Span, FirstLast) {
  // static extent
  {
    ...
    ASSERT_DEATH({ s.first<static_cast<size_t>(-1)>(); }, "");
    ASSERT_DEATH({ s.first<17>(); }, "");
    ASSERT_DEATH({ s.first<32>(); }, "");
  }

  {
    ...
    ASSERT_DEATH({ s.last<static_cast<size_t>(-1)>(); }, "");
    ASSERT_DEATH({ s.last<17>(); }, "");
    ASSERT_DEATH({ s.last<32>(); }, "");
  }
...
TEST(Span, Subspan) {
  ...
  ASSERT_DEATH({ s1.subspan(-1, 0); }, "");
  ASSERT_DEATH({ s1.subspan(16, 0); }, "");

  ASSERT_DEATH({ s1.subspan<static_cast<size_t>(-1)>(); }, "");
  ASSERT_DEATH({ s1.subspan<16>(); }, "");
}
