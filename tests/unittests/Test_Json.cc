...
TEST(Json, Null) {
  ...
  ASSERT_TRUE(IsA<Null>(Json::Load(ConstStringRef{dumped}, std::ios::binary)["key"]));
}
...
TEST(Json, DISABLED_RoundTripExhaustive) {
  ...
  // common::ParallelFor(int32_max, 12,
  //                     [&](auto i) { test(static_cast<uint32_t>(i)); });
}
...
TEST(Json, TypedArray) {
  ...
  Json::Dump(json, &str);
  {
    auto loaded = Json::Load(ConstStringRef{str});
  ...
  {
    auto loaded = Json::Load(ConstStringRef{binary}, std::ios::binary);
...
TEST(UBJson, Basic) {
  auto run_test = [](ConstStringRef str) {
